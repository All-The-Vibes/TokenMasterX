#!/usr/bin/env python
"""Headroom compression MCP server.

The model-invoked half of the Headroom layer. It exposes three tools so the host
CLI can shrink and recover content on demand:

  * ``headroom_compress(content, hint="")`` — detect the content type and compress,
    stashing the original in the CCR store so nothing is lost.
  * ``headroom_retrieve(ref)`` — expand a ``[[HR:...]]`` placeholder back to the
    exact original content.
  * ``headroom_stats()`` — report store size, dedup ratio, and retrieval counts.
...
for the inherently model-invoked ``headroom_retrieve`` escape hatch and for
...
(headroom_posttooluse.py), which runs at the append boundary before content is
...
Run: uv run --with mcp python headroom_mcp.py
"""
import os
...
# Make `import headroom` resolve when the host launches this file by absolute path
# from an arbitrary working directory (the installer copies it next to the package).
import sys as _sys
_sys.path.insert(0, str(Path(__file__).resolve().parent))

from headroom.ccr import CCR  # noqa: E402
from headroom.router import route_typed  # noqa: E402
from headroom import tokens  # noqa: E402

# Optional cap so a single compress call cannot itself return a huge blob.
MAX_RETRIEVE_CHARS = int(os.environ.get("HEADROOM_MAX_RETRIEVE", "200000"))

_STATE = {"ccr": None}


def _ccr() -> CCR:
    """Lazy-construct the CCR store on first use (honors HEADROOM_CCR env)."""
...
@mcp.tool()
def headroom_compress(content: str, hint: str = "") -> str:
    ...
    text may contain ``[[HR:...]]`` placeholders; call ``headroom_retrieve`` with
    ...
    try:
        ...
        footer = (
            f"\n\n— headroom[{ctype.value}]: {rep['tokens_before']}→{rep['tokens_after']} tok "
            f"({rep['pct_reduction']}% via {rep['backend']}); expand with headroom_retrieve —"
        )
        return compressed + footer
    except Exception as exc:  # never raise out of a tool
        return f"{content}\n\n— headroom: compression skipped ({type(exc).__name__}) —"


@mcp.tool()
def headroom_retrieve(ref: str) -> str:
    """Expand a Headroom placeholder back to its full original content.

    ``ref`` is a ``[[HR:<hash>|...]]`` token (or just the hash) that appeared in a
    ...
    try:
        ...
        return f"headroom_retrieve failed ({type(exc).__name__})."


@mcp.tool()
def headroom_stats() -> str:
    """Report Headroom cache stats: number of stored originals, on-disk vs original
    bytes (storage compression), total retrievals, and the store path."""
    try:
        s = _ccr().stats()
        if s.get("error"):
            return f"headroom store at {s.get('db_path')}: unreadable."
        ratio = s.get("compression_ratio")
        ratio_str = f"{ratio}x" if ratio else "n/a"
        return (
            f"headroom CCR store:\n"
            f"  entries:        {s['entries']}\n"
        ...
        return f"headroom_stats failed ({type(exc).__name__})."
